...
export default function Edit({ params }) {
  ...
  const fetchData = async () => {
    try {
      ...
      document.getElementById("imt").value = response.data.imt;
      document.getElementById("ket").value = response.data.ket;
  ...
  return (
    <div className="flex flex-col min-h-screen p-16 items-center justify-center">
      <div className="w-full flex flex-col text-gray-900 bg-white dark:text-white dark:bg-gray-800 rounded-lg p-8">
        ...
        <form
          onSubmit={handleSubmit}
          className="mx-auto flex-col md:hidden justify-between w-full gap-8"
        >
          <div className="flex flex-col w-full">
            <div className="mb-3">
              <label
